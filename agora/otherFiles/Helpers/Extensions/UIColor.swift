...
extension UIColor {
    ...
    static var topHeaderColor: UIColor {
        
        let colorStr = UserDefaults.standard.value(forKey: "theme") as? String
        if colorStr == nil{
            
            
            let customColorWithAlpha = Color.custom(hexString: "#14A588", alpha: 1).value // default color
            return customColorWithAlpha
        }else
        {
            let customColorWithAlpha = Color.custom(hexString: colorStr!, alpha: 1).value
            return customColorWithAlpha
        }
        
        //   return UIColor(red: 20, green: 165, blue: 136)
    }
...
extension UIView {
    func applyGradient(colours: [UIColor]) -> Void {
        self.layer.sublayers?.removeAll(where: {
            $0.classForCoder == CAGradientLayer.classForCoder()
        })
